...
class BHyveClient:
    ...
    async def _refresh_timer_programs(self, *, force_update: bool = False) -> None:
        now = time.time()
        if force_update:
            _LOGGER.debug("Forcing program refresh")
    ...
    async def get_device_history_page(
        ...
        *,
        ...
        """Get a paginated page of watering history."""
        if not force_update:
            cached = self._device_histories.get(device_id)
            if cached and page == 1 and per_page == 10:
                return cached
    ...
    async def get_landscape(
        ...
        for zone in self._landscapes.get(device_id, []):
            if str(zone.get("station")) == str(zone_id):
                return zone
        return None
